...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (embedded) {
      ...
      sessionConfig.return_url = `${origin}/editor?checkout=success&session_id={CHECKOUT_SESSION_ID}`;
    } else {
      sessionConfig.success_url = `${origin}/editor?checkout=success&session_id={CHECKOUT_SESSION_ID}`;
      sessionConfig.cancel_url = `${origin}/editor?checkout=canceled`;
    }
