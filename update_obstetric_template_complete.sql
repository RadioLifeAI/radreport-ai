-- Migration: Update US_OB_MORFOLOGICO_001 template with complete obstetric variables
...
-- 1. Update the template with comprehensive obstetric variables
...
COMMENT ON COLUMN system_templates.condicoes_logicas IS 'Array JSONB de regras condicionais suportando operadores: igual, menor_que, maior_que, menor_igual, maior_igual. Usado para alertas automáticos obstétricos.';

-- 3. Verify the update
SELECT 
  codigo, 
  titulo,
  jsonb_array_length(variaveis) as num_variaveis,
  jsonb_array_length(condicoes_logicas) as num_condicoes
